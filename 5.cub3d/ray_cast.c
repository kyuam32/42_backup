...
/*   Updated: 2021/03/30 16:48:18 by namkyu           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void ray_distance(t_data *data, double x_dir, double y_dir, char tile)
{
	...
	while (hit == 0)
	{
		...
		if (data->map.map_arr[y_map][x_map] == '2')
		{
			hit = 1;
			data->draw.tile = '2';
...
void ray_cast(t_data *data,void (*draw_target)(t_data *), char tile)
{
	...
	while (cur_dir < data->cam.FOV / 2)
	{
		...
		ray_distance(data, data->cam.dir.x, data->cam.dir.y, tile);
		draw_target(data);
...
void ray_initalize(t_data *data)
{
	...
	ray_cast(data, draw_3d, '1');
	ray_cast(data, draw_sp, '2');
	m_map_wall(data);
	ray_cast(data, draw_ray, '1');
	m_map_grid(data);
	m_map_player(data);
}
